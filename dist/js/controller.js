var addPersonApp = angular.module('addPersonApp', ['angularUtils.directives.dirPagination', 'ngMessages']);
...
addPersonApp.controller('PersonListCtrl', function ($scope, $window) {
  ...
  $scope.genderOpts = [{ value: 'Male' }, { value: 'Female' }];
  ...
  function generateString() {
    ...
    for (var i = 0; i < 10; i++) string += possible.charAt(Math.floor(Math.random() * possible.length));
    return string;
  }
  ...
  function generateGender() {
    ...
    if (isMale === 2) return 'Female';else return 'Male';
  }
